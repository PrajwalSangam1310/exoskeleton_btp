...
std_msgs::Float64 refVelocity;
void refVelocityCallback(const std_msgs::Float64& callbackMsg){
    RefRPM = callbackMsg.data; 
}

//create the nodehandle
ros::NodeHandle arduinoInterface;

//create the msg to subscribe and publish to the velocities

std_msgs::Float64 feedbackVelocity;

//create a rossubscriber to read the reference velocity
ros::Subscriber<std_msgs::Float64> refVelocitySubscriber("motor_command_velocity", refVelocityCallback);
//create the publisher to get the rotor speed value
ros::Publisher feedbackVelocityPublisher("current_motor_velocity", &feedbackVelocity);
...
arduinoInterface.advertise(feedbackVelocityPublisher);
arduinoInterface.subscribe(refVelocitySubscriber);
}
...
void loop() {
  ...
  PI_Regulator_DoWork();
  //ros part
  feedbackVelocityPublisher.publish(&feedbackVelocity);
}
