...
//ros
void refVelocityCallback(const std_msgs::Float64& refVelocity){
    RefRPM = refVellocity; 
}
...
ros::Publisher feedbackVelocityPublisher("current_motor_velocity", &feedbackVelocityVelocity);
...
void loop() {
  ...
  PI_Regulator_DoWork();
  //ros part
  feedbackVelocityPublisher(feedbackVelocity);
}
...
void PI_Regulator_DoWork() {
  ...
  if (millis() > PI_Update_Timeout) {
    ...
    //ros rpm
    feedbackVelocity = std_msgs::Float64(RPMf);
    //
