...
class Leg:
    ...
    def loadTrajectoryPoints(self, keys):
    ...
    def updteTrajectory2(self):
        ...
        for i in range(len(self.legJointTrajectory.points)): 
            ...
            self.legJointTrajectory.points[i].positions[1] = adductionjointAngles[i]        
            self.legJointTrajectory.points[i].positions[2] = adductionjointAngles[i]   
            self.legJointTrajectory.points[i].positions[3] = 0
    ...
    def leftlegJointTrajectoryClient(self):
    ...
    def rightlegJointTrajectoryClient(self):
        # Creates the SimpleActionClient, passing the type of the action
        print("before client")
        client = actionlib.SimpleActionClient('/right_leg_trajectory_controller/follow_joint_trajectory', FollowJointTrajectoryAction)
        print("after client")
        # Waits until the action server has started up and started
        # listening for goals.
        client.wait_for_server()
...
if __name__ == '__main__':
    try:
        ...
        lleg = Leg(leftjointNames)
        rleg = Leg(rightjointNames)

        leftkeysName = ['hip_adduction_l','hip_rotation_l', 'hip_flexion_l','knee_angle_l', 'ankle_angle_l']
        rightkeysName = ['hip_flexion_r', 'hip_rotation_r','hip_adduction_r','knee_angle_r', 'ankle_angle_r']
        ...
        lleg.loadTrajectoryPoints(leftkeysName)
        lleg.initiateLegJointTrajectory()
        lleg.updateLegJointTrajectory()

        rleg.loadTrajectoryPoints(rightkeysName)
        rleg.initiateLegJointTrajectory()
        rleg.updateLegJointTrajectory()


        # lleg.updteTrajectory2()
        lleg.checkTrajectory()
        result1 = lleg.leftlegJointTrajectoryClient()
        result2 = rleg.rightlegJointTrajectoryClient()
